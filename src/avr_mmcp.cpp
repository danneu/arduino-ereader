        ...
        if (send_cmd(CMD8, 0x1AA) == 1) { /* SDv2 */
            ...
            if (ocr[2] == 0x01 &&
                ...
                for (tmr = 10000; tmr && send_cmd(ACMD41, 1UL << 30); tmr--)
                    dly_100us(); /* Wait for leaving idle state (ACMD41 with HCS
                                    bit) */
                if (tmr &&
...
DRESULT disk_readp(
    ...
    if (send_cmd(CMD17, sector) == 0) { /* READ_SINGLE_BLOCK */
        ...
        if (rc == 0xFE) { /* A data block arrived */
            ...
            bc =
                512 + 2 - offset - count; /* Number of trailing bytes to skip */
...
DRESULT disk_writep(
    ...
    if (buff) { /* Send data bytes */
        ...
        if (sc) { /* Initiate sector write process */
            ...
            if ((rcv_spi() & 0x1F) ==
                0x05) { /* Receive data resp and wait for end of write process
                           in timeout of 500ms */
                for (bc = 5000; rcv_spi() != 0xFF && bc;
                     bc--) { /* Wait for ready */
                    dly_100us();
                }
                if (bc) res = RES_OK;
