...
void epd_set_lut(EPD_LUT lut) {
    ...
    for (i = 0; i < 44; i++) epd_data(lut == LUT_PARTIAL ? vcom1[i] : vcom0[i]);
    epd_cmd(LUT_WHITE_TO_WHITE);
    for (i = 0; i < 42; i++) epd_data(lut == LUT_PARTIAL ? ww1[i] : ww0[i]);
    epd_cmd(LUT_BLACK_TO_WHITE);
    for (i = 0; i < 42; i++) epd_data(lut == LUT_PARTIAL ? bw1[i] : bw0[i]);
    epd_cmd(LUT_WHITE_TO_BLACK);
    for (i = 0; i < 42; i++) epd_data(lut == LUT_PARTIAL ? wb1[i] : wb0[i]);
    epd_cmd(LUT_BLACK_TO_BLACK);
    for (i = 0; i < 42; i++) epd_data(lut == LUT_PARTIAL ? bb1[i] : bb0[i]);
}
